...
def fetch_pod_and_check_if_ready(pod_name):
    ...
    try:
        ...
        while True:
            if not _continue:
                response = coreV1Api.list_pod_for_all_namespaces(watch=False, limit=300)
            else:
                response = coreV1Api.list_pod_for_all_namespaces(watch=False, limit=300, _continue=_continue)
